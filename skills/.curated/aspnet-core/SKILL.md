...
- Personal preference: default to Minimal APIs over controller-based APIs for new greenfield projects unless the team is already familiar with MVC conventions or the API surface is large enough to benefit from controller organization.
- Personal preference: always scaffold a basic xUnit test project alongside any new ASP.NET Core project, even for small experiments — it's easier to start with tests than to add them later.
